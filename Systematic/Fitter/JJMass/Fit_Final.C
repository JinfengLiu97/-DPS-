//Jinfeng, 25.09.25
//Fitter to calculate the uncertainty from the stability of the fitter [M(JJ)]
...
void Fit_Final(float FourMuonMass_LowCut_Value, float FourMuonMass_HighCut_Value, int MassNumber, float Standard_Value, TString Output_Name) {
	...
	//Shape Parameters
	//For the purpose of testing, the shape J/psi mass peak is listed here:
	...
	//For MuMu on the mass dimension
	float J_Cheb_Co1_Value = -0.49;
	float J_Cheb_Co2_Value = -0.10;
	...
	RooRealVar N_JJ_NPP("N_JJ_NPP", "N_JJ_NPP", 500, 0, 1E6);
	RooRealVar N_JJ_NPNP("N_JJ_NPNP", "N_JJ_NPNP", 1000, 0, 1E6);

	RooRealVar N_JMuMu("N_JMuMu", "N_JMuMu", 0, 0, 1E6);
	RooRealVar N_MuMuMuMu("N_MuMuMuMu", "N_MuMuMuMu", 0, 0, 1E6);
	...
	RooArgList Parameter_List(J1Mass_, J2Mass_, x1_, x2_, FourMuonMass_, Weight_sum);
	...
	// Pseduo-dataset generate
	RooDataSet* Pseduo_Data_JJ_PP = PDF_JJ_PP.generate(RooArgSet(J1Mass_, J2Mass_, x1_, x2_), +int(N_JJ_PP.getValV()));
	RooDataSet* Pseduo_Data_JJ_NPP = PDF_JJ_NPP.generate(RooArgSet(J1Mass_, J2Mass_, x1_, x2_), +int(N_JJ_NPP.getValV()));
	RooDataSet* Pseduo_Data_JJ_PNP = PDF_JJ_PNP.generate(RooArgSet(J1Mass_, J2Mass_, x1_, x2_), +int(N_JJ_NPP.getValV()));
	RooDataSet* Pseduo_Data_JJ_NPNP = PDF_JJ_NPNP.generate(RooArgSet(J1Mass_, J2Mass_, x1_, x2_), +int(N_JJ_NPNP.getValV()));

	RooDataSet* Pseduo_Data_J1MuMu = PDF_J1MuMu.generate(RooArgSet(J1Mass_, J2Mass_, x1_, x2_), +int(N_JMuMu.getValV()));
	RooDataSet* Pseduo_Data_MuMuJ2 = PDF_J1MuMu.generate(RooArgSet(J1Mass_, J2Mass_, x1_, x2_), +int(N_JMuMu.getValV()));
	RooDataSet* Pseduo_Data_MuMuMuMu = PDF_MuMuMuMu.generate(RooArgSet(J1Mass_, J2Mass_, x1_, x2_), +int(N_MuMuMuMu.getValV()));

	float Sys = 0;
	float Max_Sys = 0;;

	//Merging
	Set->append(*Pseduo_Data_JJ_PP);
	FitResult = FittingFunction->fitTo(*Set, Extended(kTRUE), Hesse(kTRUE), Strategy(1), NumCPU(4), Save(kTRUE), Minos(kFALSE));
	Sys = (N_JJ_PP.getValV() - 2 * Standard_Value) / (2 * Standard_Value);
	Max_Sys = (fabs(Sys) > Max_Sys) ? fabs(Sys) : Max_Sys;
	
	FILE* OutputFile = NULL;
	OutputFile = fopen(Output_Name + ".txt", "a");

	fprintf(OutputFile, "%i: \n", MassNumber);
	fprintf(OutputFile, "PP append: \n");
	fprintf(OutputFile, "%i\n",FitResult->status());
	...
	Set = new RooDataSet("Set", "Set", Tree_, Parameter_List, MassCut, Weight_sum.GetName());
	Set->append(*Pseduo_Data_JJ_NPP);
	Set->append(*Pseduo_Data_JJ_PNP);
	FitResult = FittingFunction->fitTo(*Set, Extended(kTRUE), Hesse(kTRUE), Strategy(1), NumCPU(4), Save(kTRUE), Minos(kFALSE));
	Sys = (N_JJ_PP.getValV() - Standard_Value) / Standard_Value;
	Max_Sys = (fabs(Sys) > Max_Sys) ? fabs(Sys) : Max_Sys;

	OutputFile = fopen(Output_Name + ".txt", "a");

	fprintf(OutputFile, "PNP append: \n");
	fprintf(OutputFile, "%i\n", FitResult->status());
	fprintf(OutputFile, "PP: %f +/- %f\n", N_JJ_PP.getValV(), N_JJ_PP.getError());
	fprintf(OutputFile, "PNP: %f +/- %f\n", N_JJ_NPP.getValV(), N_JJ_NPP.getError());
	fprintf(OutputFile, "NPNP: %f +/- %f\n", N_JJ_NPNP.getValV(), N_JJ_NPNP.getError());
	...
	Set = new RooDataSet("Set", "Set", Tree_, Parameter_List, MassCut, Weight_sum.GetName());
	Set->append(*Pseduo_Data_JJ_NPNP);
	FitResult = FittingFunction->fitTo(*Set, Extended(kTRUE), Hesse(kTRUE), Strategy(1), NumCPU(4), Save(kTRUE), Minos(kFALSE));
	Sys = (N_JJ_PP.getValV() - Standard_Value) / Standard_Value;
	Max_Sys = (fabs(Sys) > Max_Sys) ? fabs(Sys) : Max_Sys;
	...
	fprintf(OutputFile, "NPNP append: \n");
	fprintf(OutputFile, "%i\n", FitResult->status());
	fprintf(OutputFile, "PP: %f +/- %f\n", N_JJ_PP.getValV(), N_JJ_PP.getError());
	fprintf(OutputFile, "PNP: %f +/- %f\n", N_JJ_NPP.getValV(), N_JJ_NPP.getError());
	fprintf(OutputFile, "NPNP: %f +/- %f\n", N_JJ_NPNP.getValV(), N_JJ_NPNP.getError());
	...
	Set = new RooDataSet("Set", "Set", Tree_, Parameter_List, MassCut, Weight_sum.GetName());
	Set->append(*Pseduo_Data_J1MuMu);
	Set->append(*Pseduo_Data_MuMuJ2);
	FitResult = FittingFunction->fitTo(*Set, Extended(kTRUE), Hesse(kTRUE), Strategy(1), NumCPU(4), Save(kTRUE), Minos(kFALSE));
	Sys = (N_JJ_PP.getValV() - Standard_Value) / Standard_Value;
	Max_Sys = (fabs(Sys) > Max_Sys) ? fabs(Sys) : Max_Sys;
	...
	fprintf(OutputFile, "JMuMu append: \n");
	fprintf(OutputFile, "%i\n", FitResult->status());
	fprintf(OutputFile, "PP: %f +/- %f\n", N_JJ_PP.getValV(), N_JJ_PP.getError());
	fprintf(OutputFile, "PNP: %f +/- %f\n", N_JJ_NPP.getValV(), N_JJ_NPP.getError());
	fprintf(OutputFile, "NPNP: %f +/- %f\n", N_JJ_NPNP.getValV(), N_JJ_NPNP.getError());
	...
	Set = new RooDataSet("Set", "Set", Tree_, Parameter_List, MassCut, Weight_sum.GetName());
	Set->append(*Pseduo_Data_MuMuMuMu);
	FitResult = FittingFunction->fitTo(*Set, Extended(kTRUE), Hesse(kTRUE), Strategy(1), NumCPU(4), Save(kTRUE), Minos(kFALSE));
	Sys = (N_JJ_PP.getValV() - Standard_Value) / Standard_Value;
	Max_Sys = (fabs(Sys) > Max_Sys) ? fabs(Sys) : Max_Sys;
	...
	fprintf(OutputFile, "MuMuMuMu append: \n");
	fprintf(OutputFile, "%i\n", FitResult->status());
	fprintf(OutputFile, "PP: %f +/- %f\n", N_JJ_PP.getValV(), N_JJ_PP.getError());
	fprintf(OutputFile, "PNP: %f +/- %f\n", N_JJ_NPP.getValV(), N_JJ_NPP.getError());
	fprintf(OutputFile, "NPNP: %f +/- %f\n", N_JJ_NPNP.getValV(), N_JJ_NPNP.getError());
	...
	//Plot
	//Plot J1
	RooPlot* J1Frame = J1Mass.frame(Title("M_{J/#Psi1}"));
	...
	*/
	...
	RooPlot* J1_Pull_Frame = J1Mass.frame(Title(" "));
	...
	*/
	...
	J1_Canvas->SaveAs("J1_Result.png");

	//Plot J2
	RooPlot* J2Frame = J2Mass.frame(Title("M_{J/#Psi2}"));
	...
	*/
	...
	RooPlot* J2_Pull_Frame = J2Mass.frame(Title(" "));
	...
	J2_Canvas->SaveAs("J2_Result.png");

	//Plot ctau1
	RooPlot* ctau1Frame = x1.frame(Title("c#tau^{J/#Psi1}"), Range(-0.015, 0.10));
	...
	*/
	...
	RooPlot* ctau1_Pull_Frame = x1.frame(Title(" "), Range(-0.015, 0.10));
	...
	ctau1_Canvas->SaveAs("ctau1_Result.png");

	//Plot ctau2
	RooPlot* ctau2Frame = x2.frame(Title("c#tau^{J/#Psi2}"), Range(-0.015, 0.10));
	...
	*/
	...
	RooPlot* ctau2_Pull_Frame = x2.frame(Title(" "), Range(-0.015, 0.10));
	...
	ctau2_Canvas->SaveAs("ctau2_Result.png");
	*/
}