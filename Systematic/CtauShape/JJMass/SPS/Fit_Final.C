//Jinfeng, 25.09.25
//Fitter to calculate the uncertainty from the ctau shape [M(JJ),SPS]
...
void Fit_Final(float FourMuonMass_LowCut_Value, float FourMuonMass_HighCut_Value, int MassNumber, float Standard_Value, TString Output_Name) {
	...
	//SPS ctau shape
	float ctau_JJ_Gaus_Mean_Value = 0.000019;
	float ctau_JJ_Gaus_Sigma1_Value = 0.0066;
	float ctau_JJ_Gaus_Sigma2_Value = 0.00205;
	float ctau_JJ_Gaus_Frac_Value = 0.109;
	...
	*/
	...
	fprintf(OutputFile, "%i, [%f, %f]: %i\n", MassNumber, FourMuonMass_LowCut, FourMuonMass_HighCut, FitResult->status());
